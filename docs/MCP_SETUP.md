...
- **assignLabelToUnit** - **Write action:** assign an existing project label to the source unit; enable explicitly in `prompt.config`
- **addUnitComment** - **Write action:** add a real comment to the target translation unit; use after `assignLabelToUnit` for a short MQM reason
